...
[Firestore 기록]
  컬렉션 : robot_status
  문서 ID: dsr01
  필드   : cur_flower_index (int)    ← 현재 작업 중인 꽃 번호
           job_status       (str)    ← "idle"|"running"|"paused"|"stopped"|"cancelled"
           done             (int)    ← 완료된 꽃 개수
           total            (int)    ← 전체 꽃 개수
           hw_code          (int)    ← 1:대기 2:이동 3:서보OFF 5:안전정지 6:비상정지
           hw_state         (str)    ← "STANDBY"|"MOVING"|"SAFE_OFF"|"PROT_STOP"|"EMRG_STOP"
           robot_connected  (bool)   ← 토픽 수신 기준 연결 여부 (8초 타임아웃)
...
class FirestoreLogger:
    ...
    def push_status(
        ...
        hw_code / hw_state:
          ...
          2 / "MOVING"    - 이동 중
          3 / "SAFE_OFF"  - 서보 꺼짐
          5 / "PROT_STOP" - 안전정지 (Protective Stop)
          6 / "EMRG_STOP" - 비상정지 (Emergency Stop)

        robot_connected:
          True  - 최근 8초 이내 토픽 수신 → 로봇 연결됨
          False - 8초 이상 토픽 없음       → 연결 끊김
        """
        def _write():
            try:
...
class MonitorNode(Node):
        ...
        try:
            ...
            if changed:
                ...
                if snap.hw_code == 5 and prev_hw_code != 5:
                    self._add_log("WARN", "🟡 안전정지 감지 → DB stopped 기록")
                if snap.hw_code == 6 and prev_hw_code != 6:
                    self._add_log("ERROR", "🔴 비상정지 감지 → DB stopped 기록")
                if snap.hw_code == 3 and prev_hw_code != 3:
                    self._add_log("ERROR", "⚡ 서보 꺼짐 감지 → DB stopped 기록")
                self._do_push(snap)
    ...
    def _firestore_timer_cb(self):
        ...
        # 연결 끊김도 DB 에 반영 (connected=False)
        current = (
            snap.cur_flower,
            snap.job_status,
            snap.done,
    ...
    def _do_push(self, snap: RobotSnapshot):
        ...
        self._add_log(
            ...
            f"  hw:{snap.hw_state}  conn:{snap.connected}",
        )
