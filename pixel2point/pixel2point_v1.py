...
class PixelPainter:
    ...
    def get_filled_pixel_order(self):
        ...
        # row는 아래 → 위 유지, col은 왼쪽 → 오른쪽
        for row in reversed(range(self.rows)):
            for col in range(self.cols):   # ✅ reversed 제거
        ...
        for row in reversed(range(self.rows)):
            for col in range(self.cols):   # ✅ reversed 제거
                if self.cell_colors[(row, col)] != self.empty_color:
                    ...
                    real_y = cell_h / 2 + (self.rows - 1 - row) * cell_h

                    lines.append(
                        f"    {idx-1}: [{real_x:.1f}, 100.0, {real_y+40:.1f}, 0.0, 180.0, 0.0],"
                    )
                    idx += 1
