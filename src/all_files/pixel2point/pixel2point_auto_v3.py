...
# 곡선 보정 기본값
DEFAULT_ERODE_ITER = 1          # 외곽을 살짝 줄이는 횟수
DEFAULT_TOP_THRESH = 0.55       # 최상단 판정 기준 (엄격)
DEFAULT_UPPER_THRESH = 0.40     # 상단 곡선 판정 기준
DEFAULT_MIDDLE_THRESH = 0.28    # 몸통 판정 기준
DEFAULT_LOWER_THRESH = 0.24     # 하단 판정 기준
DEFAULT_BOTTOM_THRESH = 0.18    # 꼭짓점 판정 기준 (관대)
...
class ImageProcessor:
    ...
    def shrink_black_region(self, mask: Image.Image, iterations: int) -> Image.Image:
        """
        검은 도형(0)을 안쪽으로 살짝 줄이는 역할.
        검은색 객체 / 흰색 배경 구조이므로 MaxFilter를 사용하면
        경계에 흰색이 섞인 곳이 더 쉽게 흰색으로 바뀌어 외곽이 줄어든다.
        """
        result = mask.copy()
    ...
    def _row_threshold(self, row: int, rows: int,
                       top_th: float,
                       upper_th: float,
                       middle_th: float,
                       lower_th: float,
                       bottom_th: float) -> float:
        """
        행 위치에 따라 점유율 판정 기준을 다르게 준다.
        위쪽 곡선은 엄격하게, 아래 꼭짓점은 관대하게.
        """
        if row == 0:
            return top_th
        elif row <= 2:
    ...
    def mask_to_grid_by_occupancy(
        self,
        normalized_mask: Image.Image,
        erode_iter: int,
        top_th: float,
        upper_th: float,
        middle_th: float,
        ...
        단순 resize 대신:
        1) 마스크를 살짝 축소
        2) 각 셀의 fill ratio 계산
        3) 행별 threshold 적용
        """
        corrected_mask = self.shrink_black_region(normalized_mask, erode_iter)

        small = Image.new("L", (self.cols, self.rows), 255)
        fill_ratio_map = [[0.0 for _ in range(self.cols)] for _ in range(self.rows)]

        cell_w = corrected_mask.width / self.cols
        cell_h = corrected_mask.height / self.rows
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                cell = corrected_mask.crop((x1, y1, x2, y2))
                ...
                if not pixels:
                    ...
                    fill_ratio = black_count / len(pixels)

                fill_ratio_map[row][col] = fill_ratio

                threshold = self._row_threshold(
                    row, self.rows,
                    top_th, upper_th, middle_th, lower_th, bottom_th
                )

                if fill_ratio >= threshold:
                    small.putpixel((col, row), 0)
                else:
                    small.putpixel((col, row), 255)

        return small, fill_ratio_map
    ...
    def process(
        ...
        small, fill_ratio_map = self.mask_to_grid_by_occupancy(
        ...
        for row in reversed(range(self.rows)):
            for col in range(self.cols):
                if self.cell_colors[(row, col)] != EMPTY_COLOR:
                    ...
                    lines.append(f"    {idx}: [{x:.1f}, 100.0, {z:.1f}, 0.0, 180.0, 0.0],")
                    idx += 1
        lines.append("}")
        return lines
...
class PixelPainter:
    def __init__(self, root: tk.Tk):
        ...
        # 곡선 보정 파라미터
        self.erode_iter = DEFAULT_ERODE_ITER
        self.top_th = DEFAULT_TOP_THRESH
        self.upper_th = DEFAULT_UPPER_THRESH
        self.middle_th = DEFAULT_MIDDLE_THRESH
        self.lower_th = DEFAULT_LOWER_THRESH
        self.bottom_th = DEFAULT_BOTTOM_THRESH

        self.original_image: Image.Image | None = None
        self.stage_images: dict = {}
        self.tk_previews: list = []
    ...
    def _build_controls(self):
        ...
        def slider(parent, label, from_, to, init, cmd, resolution=1):
        ...
        self.erode_scale = slider(bar, "Erode", 0, 3, self.erode_iter, self._on_erode)

        self.top_scale = slider(bar, "Top", 0.10, 0.90, self.top_th, self._on_top_th, 0.01)
        self.upper_scale = slider(bar, "Upper", 0.10, 0.90, self.upper_th, self._on_upper_th, 0.01)
        self.middle_scale = slider(bar, "Middle", 0.05, 0.90, self.middle_th, self._on_middle_th, 0.01)
        self.lower_scale = slider(bar, "Lower", 0.05, 0.90, self.lower_th, self._on_lower_th, 0.01)
        self.bottom_scale = slider(bar, "Bottom", 0.05, 0.90, self.bottom_th, self._on_bottom_th, 0.01)
    ...
    def _on_margin(self, value):
        self.margin = int(float(value))
        self._update_info()
        if self.original_image:
            self._process_image()

    def _on_erode(self, value):
        self.erode_iter = int(float(value))
        self._update_info()
        if self.original_image:
            self._process_image()

    def _on_top_th(self, value):
        self.top_th = float(value)
        self._update_info()
        if self.original_image:
            self._process_image()

    def _on_upper_th(self, value):
        self.upper_th = float(value)
        self._update_info()
        if self.original_image:
            self._process_image()

    def _on_middle_th(self, value):
        self.middle_th = float(value)
        self._update_info()
        if self.original_image:
            self._process_image()

    def _on_lower_th(self, value):
        self.lower_th = float(value)
        self._update_info()
        if self.original_image:
            self._process_image()

    def _on_bottom_th(self, value):
        self.bottom_th = float(value)
        self._update_info()
        if self.original_image:
            self._process_image()
    ...
    def _update_previews(self):
        ...
        self.tk_previews = []
        for canvas, pil_img in zip(self.preview_canvases, pil_images):
    ...
    def _show_process_log(self):
        ...
        self.text.insert(tk.END, (
            ...
            f"  6. 외곽 축소(erosion 유사) 횟수: {self.erode_iter}\n"
            f"  7. 셀 점유율 기준으로 {COLS}×{ROWS} 격자화\n"
            f"     - top:    {self.top_th:.2f}\n"
            f"     - upper:  {self.upper_th:.2f}\n"
            f"     - middle: {self.middle_th:.2f}\n"
            f"     - lower:  {self.lower_th:.2f}\n"
            f"     - bottom: {self.bottom_th:.2f}\n"
            f"  8. 좌우 대칭 보정: {sym_text}\n\n"
            "[픽셀 미리보기]\n"
        ))
        self.text.insert(tk.END, self.grid.to_text_preview())

        if "fill_ratio_map" in self.stage_images:
            self.text.insert(tk.END, "\n\n[셀 점유율(fill ratio)]\n")
            ratio_map = self.stage_images["fill_ratio_map"]
            for row in range(ROWS):
                line = " ".join(f"{ratio_map[row][col]:.2f}" for col in range(COLS))
                self.text.insert(tk.END, line + "\n")

        self.text.insert(tk.END, (
            "\n[추천값]\n"
            "- 1번 하트처럼 윗곡선을 더 줄이고 싶으면:\n"
            "  Top, Upper 값을 올리세요.\n"
            "- 전체적으로 너무 두껍게 나오면:\n"
            "  Erode를 1 → 2로 올리세요.\n"
            "- 하단 꼭짓점이 사라지면:\n"
    ...
    def _update_info(self):
        ...
        self.info_label.config(
            text=(
                f"색상: {self.current_color} | "
                f"threshold: {self.threshold} | margin: {self.margin} | "
                f"erode: {self.erode_iter} | symmetry: {sym}"
            )
        )
